...
export default function ChatPageShell({
  ...
  useEffect(() => {
    fetchSuggestions()
      .then((items) => setSuggestions(items.length > 0 ? items : DEFAULT_SUGGESTIONS))
  ...
  const handleSend = useCallback(
    ...
    [updateAssistantMessage, selectedProduct, backendMode, isLoading, isInitializing, historyLoading],
  );
  ...
  const handleViewDetail = useCallback(
    ...
    [updateAssistantMessage, parsedProductUrls, backendMode, isLoading, isInitializing, historyLoading],
  );

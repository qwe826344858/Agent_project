...
const CONFIGURED_DEEP_AGENT_CHAT_URL =
  process.env.NEXT_PUBLIC_DEEP_AGENT_CHAT_URL?.trim() || "/api/agent/deep-chat";
const CONFIGURED_RAG_AGENT_CHAT_URL =
  process.env.NEXT_PUBLIC_RAG_AGENT_CHAT_URL?.trim() || "/chat/rag-agent";
...
function getChatEndpointUrl(backendMode: ChatBackendMode): string {
  switch (backendMode) {
    case "agent_chat":
      return normalizeEndpointUrl(CONFIGURED_AGENT_CHAT_URL);
    case "deep_agent_chat":
      return normalizeEndpointUrl(CONFIGURED_DEEP_AGENT_CHAT_URL);
    case "rag_agent_chat":
      return normalizeEndpointUrl(CONFIGURED_RAG_AGENT_CHAT_URL);
    case "legacy_chat":
    default:
      return normalizeEndpointUrl(CONFIGURED_LEGACY_CHAT_URL);
  }
}
...
/** 使用 DeepAgent 实验链路发送聊天消息 */
export async function sendDeepAgentChatMessage(
  message: string,
  handlers: SSEHandlers,
  options?: Omit<SendChatMessageOptions, "backendMode">
): Promise<void> {
  return sendChatMessage(message, handlers, {
    ...options,
    backendMode: "deep_agent_chat",
  });
}

/** 使用 RAG Agent 知识库链路发送聊天消息 */
export async function sendRAGAgentChatMessage(
  message: string,
  handlers: SSEHandlers,
  options?: Omit<SendChatMessageOptions, "backendMode">
): Promise<void> {
